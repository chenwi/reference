...
{
	bsw2opt_t *opt; ///存储各种参数的(程序参数)
	bwaidx_t *idx; ///这个是存储bwt索引+参考基因组，N随机存的
	int c;

	opt = bsw2_init_opt();   ///各类参数初始化
	srand48(11);
	...
	if ((idx = bwa_idx_load(argv[optind], BWA_IDX_BWT|BWA_IDX_BNS)) == 0)  ///读入索引和基因组吧
        return 1;
