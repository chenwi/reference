...
bwa_seqio_t *bwa_seq_open(const char *fn)
{
	...
	///给一个bwa_seqio_t数据结构分配空间
	bs = (bwa_seqio_t*)calloc(1, sizeof(bwa_seqio_t));
	///打开文件
	fp = xzopen(fn, "r");
	///这个应该是让bwa_seqio_t结构体将这个已打开的文件指正记录下来
	bs->ks = kseq_init(fp);  ///这些乱起八早的东西都是宏定义，在kseq.h文件内可以看到
...
{
    ///n_needed似乎是一个挺大的值，应该reads是按一个一个batch读进来
    ///*n应当是实际读入的reads数目
	bwa_seq_t *seqs, *p;
	kseq_t *seq = bs->ks;
