 ...
 * process_seqs() to realize multi-threading */
        ...
        ///_seq是reads信息
		///bsw2opt_t *opt是各种参数
        ///bntseq_t *bns据说是基因组的各种信息，如有多少个染色体这种
        ///bwt_t * const target这个是FM-index
        ///pac是基因组
        ///is_pe是否双端比对


static void bsw2_aln_core(bsw2seq_t *_seq, const bsw2opt_t *_opt, const bntseq_t *bns,
                          uint8_t *pac, const bwt_t *target, int is_pe)
{
	int x;
	bsw2opt_t opt;
	...
	for (x = 0; x < _seq->n; ++x) {
		bsw2seq1_t *p = _seq->seq + x; ///p代表当前reads的所有信息
		...
		seq[0] = calloc(l * 4, 1);
		seq[1] = seq[0] + l; ///这个是加L,不是加1；分配四倍空间的原因是read 1和read 2的正向和反向互补链共四条
...
static void process_seqs(bsw2seq_t *_seq, const bsw2opt_t *opt, const bntseq_t *bns,
                         uint8_t *pac, const bwt_t *target, int is_pe)
{
...
void bsw2_aln(const bsw2opt_t *opt, const bntseq_t *bns, bwt_t * const target, const char *fn, const char *fn2)
{
	...
	kseq_t *ks, *ks2;
	int l, is_pe = 0, i, n;
	uint8_t *pac;   ///pac是基因组(2-bits 存储方式)
	bsw2seq_t *_seq;  ///reads
	bseq1_t *bseq;

    ///fprintf(stderr, "the length of genome =%u\n",bns->l_pac);
    ...
    ///fprintf(stderr, "chunk_size =%u bp\n",opt->chunk_size);
	...
	while ((bseq = bseq_read(opt->chunk_size * opt->n_threads, &n, ks, ks2)) != 0) {
		...
		int size = 0;  ///size是总共多少bp，不是reads的个数
		fprintf(stderr, "n =%u\n",n);
		fprintf(stderr, "_seq->max =%u\n",_seq->max);
